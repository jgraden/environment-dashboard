...
/**
 * This class is a data access object that handles 
 * interactions between the application and the db.
...
public class DashboardDAO {
	 ...
	 * @exception SQLException - unable to execute create dashboard query. 
	 */
	public boolean createDashboardTable() throws SQLException{
		
		//Get DB connection
		Connection conn = DBConnection.getConnection();
		return conn.prepareStatement(DashboardDAO.createDashboardTblQuery).execute();
	}
	
	/**
	 * Add build to environment dashboard.
	 * @param index 		- the primary key in the env_dashboard table.
	 * @param build 		- the build being added to the dashboard.
	 * @param componant     - the component being deployed/built
	 * @param buildJobUrl   - the URL of the build job.
	 * @return 				- true if build added else false.
	 * @throws SQLException - unable to execute insert build query.
	 */
	public boolean addBuild(String index, Build build, String componant, String buildJobUrl) throws SQLException {
		
		// Get DB Connection
		Connection conn = DBConnection.getConnection();
		PreparedStatement stat = conn.prepareStatement(DashboardDAO.insertBuildQuery);
	 ...
	 * @param index 		- primary key in the env_dashboard table
	 * @param build 		- the build being added to the dashboard
	 * @return      		- true if build updated else false
	 * @throws SQLException - unable to execute update build query.
	 */
	public boolean updateBuild(String index, Build build) throws SQLException{
		// Get DB Connection
		Connection conn = DBConnection.getConnection();
		PreparedStatement stat = conn.prepareStatement(DashboardDAO.updateBuildQuery);
	 ...
	 * @param daysOld 		- delete build in the last days i.e. 30
	 * @return				- true if builds deleted else false.
	 * @throws SQLException - unable to execute delete build query.
	 */
	public boolean deleteBuild(int daysOld) throws SQLException{
		...
		if (daysOld > 0){
			// Get DB Connection
			Connection conn = DBConnection.getConnection();
			PreparedStatement stat = conn.prepareStatement(DashboardDAO.deleteOldBuildQuery + daysOld);
